...
public class StoveView9W70 extends FrameLayout implements IStove {
    Context cx;
    View viewRoot;

    @InjectView(R.id.view_left)
    Stove9W70ChildrenView viewLeft;
    ...
    public StoveView9W70(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context, attrs);
    }

    public StoveView9W70(Context context) {
        super(context);
        initView(context, null);
    }


    public StoveView9W70(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        viewRoot = LayoutInflater.from(cx).inflate(
                R.layout.stove_view_9w70, this, true);
    ...
    private void init() {
        ...
        lockLayout.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                stove = Utils.getDefaultStove();
                if (null == stove[0]) return;
                if (!StoveView9W70.this.stove[0].isConnected()) {
                    ToastUtils.show(R.string.device_off_line);
                    return;
                }

                if (StoveView9W70.this.stove[0].leftHead.status == StoveStatus.Off && StoveView9W70.this.stove[0].rightHead.status == StoveStatus.Off) {
    ...
    private void dealEvent(final Short headId, String tag) {
        stove = Utils.getDefaultStove();
        if (null == stove[0]) return;
        LogUtils.e("20190524", "dealEvent stove[]" + stove.hashCode() + " isConnected:" + stove[0].isConnected());
        //模拟童锁开启
        if (stove[0].isLock) {
            lockStart.startWaveOne();
            return;
