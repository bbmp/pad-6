...
public class PulToLeftViewGroupl extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    public void completeToUpload() {
        try {
            ...
            ToastUtils.show(R.string.cook_load_finish_text, 0);
        ...
        for (int i = 0; i < mMoveViews.size(); i++) {
            if (i < mMoveRects.size()) {
                if (mMoveRects.get(i) != null) {
                    final int indexAt = i;
                    TranslateAnimation anims = new TranslateAnimation(mMoveViews.get(i).getRight(),
                            mMoveRects.get(i).right, 0, 0);
                    anims.setDuration(ANIM_TIME);
                    mMoveViews.get(i).startAnimation(anims);
                    mMoveViews.get(indexAt).layout(mMoveRects.get(indexAt).left, mMoveRects.get(indexAt).top,
                            mMoveRects.get(indexAt).right, mMoveRects.get(indexAt).bottom);
                }
            }
        }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        boolean isTouchOutOfScrollView = ev.getX() >= originalRect.right || ev.getX() <= originalRect.left;
        //如果当前view的Y上的位置
                    ...
                    for (int i = 0; i < mMoveViews.size(); i++) {
                        if (i < mMoveRects.size()) {
                            if (mMoveViews.get(i) != null && mMoveRects.get(i) != null) {
                                mMoveViews.get(i).layout(mMoveRects.get(i).left + offset, mMoveRects.get(i).top, mMoveRects.get(i).right + offset, mMoveRects.get(i).bottom);
                            }
                        }
                    }
                    isMoved = true;
