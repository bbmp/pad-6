...
public class PatchUtils {
    ...
    public static void atTimeAndFix(Integer[] index) throws Exception {
        if (Plat.DEBUG)
            LogUtils.i("20170811", "计数器：" + index[0]);
        if (index[0] >= 0 && index[0] < (1800 * 24))
            return;
        index[0] = 0;
        Random random = new Random();
        int num = random.nextInt(40);
        if (Plat.DEBUG)
            LogUtils.i("20170811", "秒后开始 检测：：" + (num * 3) * 1000);
        new Timer().schedule(new TimerTask() {
            ...
            public void run() {
                try {
                    ...
                    if (e != null)
                        LogUtils.i("20170811", "发生错误：：" + e.getMessage());
                    LogUtils.i("20170811", "发生错误——————");
                }
            }
        }, num * 1000);
    }
