...
public abstract class AbsRStepCook extends AbsService implements IRecipeStep {
    ...
    protected final int MAX_SENDPARAM_NUM = 10;//最大下发次数
    ...
    void listToTask() {
        if (mStep_CookingMap == null || mStep_CookingMap.size() <= 0) {
            LogUtils.i("20171110", "步骤实例集合null");
        } else {
            Iterator iterator = mStep_CookingMap.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry entry = (Map.Entry) iterator.next();
                //遍历设备菜谱表 若有遗留设备则清空
                LogUtils.i("20171110", "步骤实例集合:" + entry.getKey() + "  " + entry.getValue());
            }
        }
    }
    ...
    public void run(String id) {
        ...
        LogUtils.i("startCooking", "run:" + ids[0] + "  " + (ids.length > 1 ? ids[1] : null));
        ...
        setDeviceCommand(new VoidCallback4() {
            ...
            public void onSuccess() {
                ...
                LogUtils.i("stove_sts", "下发指令成功");
            }

            @Override
            public void onFailure(Object t) {
                LogUtils.i("20180509", "t::" + t);
                mWaitFlag = false;
     ...
     * @param isExit  是否退出菜谱*/
    @Override
    public void close(boolean isOff,boolean isExit) {
...
//        this.dispose();
    ...
    //关闭灶具
    protected void closeRRQZ(boolean isExit){
    ...
    protected void startCountdown(final int needTime) {
        ...
        mFuture = TaskService.getInstance().scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
                LogUtils.i("20171108", "startCountdown mRemainTime " + mRemainTime);
                mIsCounting = true;
                if (mRemainTime <= 0) {
                    stopCountdown();
                    onCountComplete();
    ...
    protected boolean ifContainDevice(String guid) {
        return getDeviceRecipeMap().get(guid) == null ? false : true;
    }
