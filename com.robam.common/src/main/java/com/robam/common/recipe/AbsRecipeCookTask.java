...
public class AbsRecipeCookTask extends AbsService implements IAbsRecipeCookInterface, Runnable {
    ...
    protected void recipeDispatch(int stepIndex) {
        ...
        if (StringUtils.isNullOrEmpty(cookStep.getDc())
            ...
            if (mCurrentStep != null)
                mCurrentStep.close(false, true);
            //当前步骤没有设备
            mViewCallBack.ondisplay(mCurrentIndex, null);
        } else {
            if (mCurrentStep != null) {
                LogUtils.i("20180330", "mCurrentIndex::" + mCurrentIndex + "  cookSteps.size():" + cookSteps.size());
                mCurrentStep.close(false, mCurrentIndex + 2 < cookSteps.size()); //mCurrentIndex+2 是用判断是否是在最后一步退出
            }
            LogUtils.i("startCooking", "stepIndex:" + stepIndex);
    ...
    public void checkDeviceBefore(CookStep cookStep, final int stepIndex) {
        mViewCallBack.onPreStartCheck(prerun(cookStep), stepIndex, new RecipeDeviceSelect<String>() {
            @Override
            public void selectDevice(String id, String... para) {
                ...
                if (Utils.isStove(id)) {
//                    LogUtils.i("App_Service","id:"+para[0]);
...
//                recipeStep.refreshInit();
    ...
    public Map<String, Object> prerun(CookStep cookStep) {
        ...
        if (IAppType.RKPBD.equals(Plat.appType)) {
            deviceList.add(Utils.getDefaultStove()[0]);
            ...
            deviceList.addAll(Utils.getAllStoves());
            deviceList.addAll(Utils.getAllOven());
            deviceList.addAll(Utils.getAllSteams());
            deviceList.addAll(Utils.getAllMicro());
            deviceList.addAll(Utils.getAllSteamOvenOne());
        }
        ...
        mPreRunMap.put(IRecipe.DEVICE_AVAILABLE, avai_list);
        return mPreRunMap;
    }
    ...
    protected boolean ifContainDevice(IDevice device) {
        ...
        return false*/
        ;
        return AbsRStepCook.getDeviceRecipeMap().get(device.getID()) == null ? false : true;
    }
    ...
    @Override
    public void close(boolean isTempExit) {
        if (mCurrentStep != null) {
            mCurrentStep.close(true, isTempExit);
        }
    ...
    public interface RecipeDeviceSelect<T extends String> {
         ...
         * 选择设备 t 具体guid
         */
        void selectDevice(T t, String... head);
    ...
    protected class RecipeStepCall implements IStepCallback {
        ...
        public void onSet(Throwable t) {
            mViewCallBack.onPreSet(t, t != null ? null : new RecipeDeviceSelect() {
                @Override
                public void selectDevice(String s, String... head) {//s值在此处可null
